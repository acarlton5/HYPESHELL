...
Item {
    ...
    property MprisPlayer player: visiblePlayers.length > 0 ? visiblePlayers[playerIndex % visiblePlayers.length] : null
    ...
    function dedupeKey(p) {
        const entry = (p?.desktopEntry ?? "").toString().toLowerCase()
        const title = (p?.trackTitle ?? "").toString().toLowerCase()
        const artist = (p?.trackArtist ?? "").toString().toLowerCase()
        const album = (p?.trackAlbum ?? "").toString().toLowerCase()
        return `${entry}|${title}|${artist}|${album}`
    }

    function shouldReplacePlayer(current, candidate) {
        if (!current) return true
        const currentPlaying = current.playbackState === MprisPlaybackState.Playing
        const candidatePlaying = candidate.playbackState === MprisPlaybackState.Playing
        if (candidatePlaying !== currentPlaying) return candidatePlaying
        const currentCanRaise = current?.canRaise ?? false
        const candidateCanRaise = candidate?.canRaise ?? false
        if (candidateCanRaise !== currentCanRaise) return candidateCanRaise
        return false
    }

    function rebuildVisiblePlayers() {
        const selectedByKey = {}
        const keyOrder = []
        const sourcePlayers = Mpris.players.values

        for (const p of sourcePlayers) {
            const key = dedupeKey(p)
            if (!(key in selectedByKey)) {
                selectedByKey[key] = p
                keyOrder.push(key)
                continue
            }
            if (shouldReplacePlayer(selectedByKey[key], p)) {
                selectedByKey[key] = p
            }
        }

        visiblePlayers = keyOrder.map((key) => selectedByKey[key])
        playerIndex = visiblePlayers.length === 0 ? 0 : playerIndex % visiblePlayers.length
    }
    ...
    Component.onCompleted: rebuildVisiblePlayers()
    Connections { target: Mpris.players; function onValuesChanged() { rebuildVisiblePlayers() } }
    ...
    CycleArrow {
        ...
        arrowText: "‹"
        visible: visiblePlayers.length > 1
    ...
    Column {
        ...
        PlayerDots { width: parent.width; visible: root.showPlayerDots && visiblePlayers.length > 1 }
    }
