...
Singleton {
    ...
    function resolveThemeWallpaperFolder(themeName) {
        ...
        const folder = Directories.home + "/.config/hype/themes/" + name + "/wallpapers"
        return FileUtils.trimFileProtocol(folder)
    }
    ...
    Process {
        ...
        command: [
            "bash",
            "-lc",
            "grep '^hypeTheme=' \"$HOME/.config/hype/hype.conf\" | cut -d'\"' -f2 | head -n1"
        ]
