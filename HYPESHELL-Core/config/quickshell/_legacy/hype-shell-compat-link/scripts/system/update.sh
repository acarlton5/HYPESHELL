...
# Paths / repo
CONFIG="$HOME/.config/hype/config/configuration.json"
QS_DIR="$HOME/.config/quickshell/hype-shell"
REPO="${HYPE_SHELL_REPO:-morph/hype-shell}"
...
# Download
run "Downloading Hype Shell $latest" \
    curl -fsSL \
...
root_dir="$(find "$tmp" -maxdepth 1 -mindepth 1 -type d | head -n1)"
[[ -n "$root_dir" && -d "$root_dir" ]] || fail "Could not locate extracted archive root"

if [[ -d "$root_dir/quickshell/hype-shell" ]]; then
    SRC_DIR="$root_dir/quickshell/hype-shell"
else
    SRC_DIR="$(find "$root_dir/quickshell" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | head -n1)"
fi

[[ -n "$SRC_DIR" && -d "$SRC_DIR" ]] || fail "Could not locate quickshell shell directory in archive"
...
run "Reloading shell" bash -c "
    ...
    nohup qs -c hype-shell &>/dev/null & disown
"

printf "[✓] Updated Hype Shell: %s -> %s\n" "$current" "$latest"
