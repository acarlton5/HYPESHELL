...
list_kind() {
  ...
  combined_index | jq -c \
    ...
    ' | while IFS= read -r entry; do
      ...
      installed=false
      [[ -f "$root/$package_id/manifest.json" ]] && installed=true
...
entry_json() {
  ...
  local id="$2"
  combined_index | jq -e --arg packageType "$package_type" --arg id "$id" 'first(.[] | select(.packageType == $packageType and .id == $id))'
}
...
install_entry() {
  ...
  [[ -f "$target/manifest.json" ]] || { echo "Installed package has no manifest.json: $target" >&2; exit 1; }
