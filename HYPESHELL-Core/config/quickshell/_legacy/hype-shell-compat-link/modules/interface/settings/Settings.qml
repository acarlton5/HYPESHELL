...
Scope {
    ...
    IpcHandler {
        function open(menu: string) {
            ...
            const rawMenu = String(menu || "").toLowerCase()
            const normalizedMenu = rawMenu === "module" || rawMenu === "modules" || rawMenu === "hypemodule" || rawMenu === "hypemodules"
                ? "HYPEMODULES"
                : (rawMenu === "gadget" || rawMenu === "gadgets" || rawMenu === "hypegadget" || rawMenu === "hypegadgets"
                    ? "HYPEGADGETS"
                    : String(menu || ""))

            if (normalizedMenu !== "" && settingsWindow !== null) {
                for (var i = 0; i < settingsWindow.menuModel.length; i++) {
                    var item = settingsWindow.menuModel[i];
                    if (!item.header && item.label.toLowerCase() === normalizedMenu.toLowerCase()) {
    ...
    LazyLoader {
        ...
        Window {
            ...
            property var menuModel: [
                ...
                { "icon": "toys", "label": "HYPEMODULES", "page": 8 },
                { "icon": "widgets", "label": "HYPEGADGETS", "page": 9 },
                { "icon": "hub", "label": "DSS Systems", "page": 10 },
            ...
            Item {
                ...
                StackLayout {
                    ...
                    ModulesConfig         { Layout.fillWidth: true; Layout.fillHeight: true; viewMode: "modules" }
                    ModulesConfig         { Layout.fillWidth: true; Layout.fillHeight: true; viewMode: "gadgets" }
                    DssWorkspaceConfig    { Layout.fillWidth: true; Layout.fillHeight: true }
