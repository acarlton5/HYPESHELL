...
ContentMenu {
    ...
    title: modulesView ? "HYPEMODULES" : "HYPEGADGETS"
    description: modulesView
        ? "Manage HypeShell HYPEMODULE packages."
        : "Manage HypeShell HYPEGADGET packages."
    ...
    readonly property string storeSourcePath: Directories.home + "/.config/hype/store/sources.json"
    readonly property string storeScript: Directories.scriptsPath + "/store/hype-store.sh"
    ...
    property var storeModulesModel: []
    property bool storeModulesLoading: false
    property string storeModulesError: ""

    property var storeGadgetsModel: []
    property bool storeGadgetsLoading: false
    property string storeGadgetsError: ""
    ...
    function parseStoreModulesPayload(rawText) {
        const payload = String(rawText || "").trim()
        if (payload.length === 0) {
            storeModulesModel = []
            return
        ...
        try {
            const parsed = JSON.parse(payload)
            storeModulesModel = Array.isArray(parsed) ? parsed : []
            if (!Array.isArray(parsed))
                storeModulesError = "Store modules payload was not an array."
        } catch (error) {
            storeModulesModel = []
            storeModulesError = "Failed to parse store modules: " + error
        }
    }

    function parseStoreGadgetsPayload(rawText) {
        const payload = String(rawText || "").trim()
        if (payload.length === 0) {
            storeGadgetsModel = []
            return
        ...
        try {
            const parsed = JSON.parse(payload)
            storeGadgetsModel = Array.isArray(parsed) ? parsed : []
            if (!Array.isArray(parsed))
                storeGadgetsError = "Store gadgets payload was not an array."
        } catch (error) {
            storeGadgetsModel = []
            storeGadgetsError = "Failed to parse store gadgets: " + error
        }
    }
    ...
    function refreshStoreModules() {
        if (storeModulesLoading)
            return

        storeModulesLoading = true
        storeModulesError = ""
        storeModulesListProc.running = true
    }

    function refreshStoreGadgets() {
        if (storeGadgetsLoading)
            return

        storeGadgetsLoading = true
        storeGadgetsError = ""
        storeGadgetsListProc.running = true
    }
    ...
    function storeKindKey(kindLabel) {
        return kindLabel === "gadget" ? "HYPEGADGET" : "HYPEMODULE"
    }

    function storeAction(action, kindLabel, packageId) {
        const kind = root.storeKindKey(kindLabel)
        pendingStoreKind = kind
        storeActionProc.command = [root.storeScript, action, kind, packageId]
        storeActionProc.running = true
    }
    ...
    Process {
        id: storeModulesListProc
        command: [root.storeScript, "list", "HYPEMODULE"]
        running: false

        stdout: StdioCollector {
            onStreamFinished: root.parseStoreModulesPayload(text)
        }
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                const msg = String(text || "").trim()
                if (msg.length > 0)
                    root.storeModulesError = msg
            }
        }

        onExited: function(exitCode) {
            root.storeModulesLoading = false
            if (exitCode !== 0 && root.storeModulesError.length === 0)
                root.storeModulesError = "Could not read HypeStore modules."
        }
    }

    Process {
        id: storeGadgetsListProc
        command: [root.storeScript, "list", "HYPEGADGET"]
        running: false

        stdout: StdioCollector {
            onStreamFinished: root.parseStoreGadgetsPayload(text)
        }
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                const msg = String(text || "").trim()
                if (msg.length > 0)
                    root.storeGadgetsError = msg
            }
        }

        onExited: function(exitCode) {
            root.storeGadgetsLoading = false
            if (exitCode !== 0 && root.storeGadgetsError.length === 0)
                root.storeGadgetsError = "Could not read HypeStore gadgets."
        }
    }
    ...
    Process {
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                const msg = String(text || "").trim()
                if (msg.length > 0) {
                    if (root.pendingStoreKind === "HYPEGADGET")
                        root.storeGadgetsError = msg
                    else
                        root.storeModulesError = msg
                }
        ...
        onExited: function(exitCode) {
            if (exitCode === 0) {
                root.refreshAll()
            } else if (root.pendingStoreKind === "HYPEGADGET" && root.storeGadgetsError.length === 0) {
                root.storeGadgetsError = "HypeStore action failed."
            } else if (root.storeModulesError.length === 0) {
                root.storeModulesError = "HypeStore action failed."
            }

            root.pendingStoreKind = ""
        }
    }
    ...
    ContentCard {
        ColumnLayout {
            ...
            StyledText {
                text: "HYPEMODULE folder: " + root.shellModulesRoot
            ...
            StyledText {
                text: "HYPEMODULE local registry: " + root.shellModulesRegistryPath
                wrapMode: Text.WrapAnywhere
                font.pixelSize: Metrics.fontSize("small")
                color: Appearance.m3colors.m3onSurfaceVariant
                visible: root.modulesView
            ...
            StyledText {
                text: "HYPEGADGET folder: " + root.gadgetsRoot
                wrapMode: Text.WrapAnywhere
                font.pixelSize: Metrics.fontSize("small")
                color: Appearance.m3colors.m3onSurfaceVariant
            ...
            StyledText {
                text: modulesView
                    ? "HYPEMODULE packages change shell behavior and UI features."
                    : "HYPEGADGET packages render widgets on the desktop stage."
            ...
            RowLayout {
                ...
                StyledButton {
                    ...
                    icon: "refresh"
                    enabled: !root.shellModulesLoading
                        && !root.gadgetsLoading
                        && !root.storeModulesLoading
                        && !root.storeGadgetsLoading
                    onClicked: root.refreshAll()
                }

                StyledButton {
                    text: "Open HYPEMODULE Folder"
                    icon: "folder_open"
                    secondary: true
            ...
            StyledText {
                text: "Store source: " + root.storeSourcePath
                wrapMode: Text.WrapAnywhere
                font.pixelSize: Metrics.fontSize("small")
                color: Appearance.m3colors.m3onSurfaceVariant
            }
    ...
    ContentCard {
        ...
        StyledText {
            text: "HYPEMODULES: " + root.shellModulesError
            wrapMode: Text.WordWrap
    ...
    ContentCard {
        visible: root.modulesView && root.storeModulesError.length > 0

        StyledText {
            text: "HypeStore: " + root.storeModulesError
            wrapMode: Text.WordWrap
            color: Appearance.m3colors.m3error
        }
    }
    ...
    ContentCard {
        ...
        StyledText {
            text: "HYPEGADGETS: " + root.gadgetsError
            wrapMode: Text.WordWrap
            color: Appearance.m3colors.m3error
        }
    }
    ...
    ContentCard {
        visible: root.gadgetsView && root.storeGadgetsError.length > 0

        StyledText {
            text: "HypeStore: " + root.storeGadgetsError
            wrapMode: Text.WordWrap
            color: Appearance.m3colors.m3error
        }
    }
    ...
    ContentCard {
        visible: root.modulesView && root.storeModulesModel.length > 0

        StyledText {
            text: "Available From HypeStore"
    ...
    Repeater {
        model: root.modulesView ? root.storeModulesModel : []

        delegate: StoreEntryCard {
            storeData: modelData
            kindLabel: "module"
        }
    }
    ...
    ContentCard {
        visible: root.gadgetsView && root.storeGadgetsModel.length > 0
    ...
    Repeater {
        model: root.gadgetsView ? root.storeGadgetsModel : []
    ...
    ContentCard {
        ...
        StyledText {
            text: root.modulesView ? "No HYPEMODULE packages found." : "No HYPEGADGET packages found."
            wrapMode: Text.WordWrap
            color: Appearance.m3colors.m3onSurfaceVariant
        }
    }
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                StyledButton {
                    ...
                    icon: "delete"
                    secondary: true
                    onClicked: root.storeAction("uninstall", kindLabel, moduleData.id)
                }
            }
            ...
            StyledText {
                ...
                text: (kindLabel === "module" ? "HYPEMODULE" : "HYPEGADGET") + " disabled. Enable it to configure settings."
                color: Appearance.m3colors.m3onSurfaceVariant
            }
    ...
    component StoreEntryCard: ContentCard {
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                StyledText {
                    text: storeData.name + " (" + storeData.id + ")"
                    font.pixelSize: Metrics.fontSize(18)
                    font.bold: true
                }
                ...
                StyledText {
                    ...
                    wrapMode: Text.WordWrap
                    color: Appearance.m3colors.m3onSurfaceVariant
                    visible: String(storeData.description || "").length > 0
                }

                StyledText {
                    text: "Version: " + (storeData.version || "unknown")
                        + "    Source: " + (storeData.sourceName || storeData.sourceId || "unknown")
                    font.pixelSize: Metrics.fontSize("small")
                    color: Appearance.m3colors.m3onSurfaceVariant
                }
                ...
                StyledText {
                    ...
                    font.pixelSize: Metrics.fontSize("small")
                    color: Appearance.m3colors.m3onSurfaceVariant
                    visible: String(storeData.repo || "").length > 0
                }
            }

            StyledButton {
                text: storeData.installed ? "Reinstall" : "Install"
                icon: "download"
                secondary: storeData.installed
                Layout.preferredWidth: 140
                onClicked: root.storeAction("install", kindLabel, storeData.id)
            }
        }
    }
