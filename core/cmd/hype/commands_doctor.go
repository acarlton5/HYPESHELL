...
import (
	...
	"github.com/acarlton5/HypeShell/core/internal/server/brightness"
	"github.com/acarlton5/HypeShell/core/internal/server/network"
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "Unsupported distribution") {
			...
			case osRelease["PRETTY_NAME"] != "":
				message = fmt.Sprintf("%s (not supported by hype setup)", osRelease["PRETTY_NAME"])
...
func checkVersions(qsMissingFeatures bool) []checkResult {
	hypeCliPath, _ := os.Executable()
	hypeCliDetails := ""
	if doctorVerbose {
		hypeCliDetails = hypeCliPath
	}

	results := []checkResult{
		{catVersions, "Hype CLI", statusOK, formatVersion(Version), hypeCliDetails, doctorDocsURL + "#hype-cli"},
	}
	...
	hypeVersion, hypePath := getHYPEShellVersion()
	if hypeVersion != "" {
		results = append(results, checkResult{catVersions, "HypeShell Shell", statusOK, hypeVersion, hypePath, doctorDocsURL + "#hype-shell"})
	} else {
		results = append(results, checkResult{catVersions, "HypeShell Shell", statusError, "Not installed or not detected", "Run './install.sh' to install", doctorDocsURL + "#hype-shell"})
	}

	return results
}

func getHYPEShellVersion() (version, path string) {
	...
	if hypePath, err := config.LocateHYPEConfig(); err == nil {
		versionFile := filepath.Join(hypePath, "VERSION")
		if data, err := os.ReadFile(versionFile); err == nil {
			return strings.TrimSpace(string(data)), hypePath
		}
		return "installed", hypePath
	}
...
func checkHYPEInstallation() []checkResult {
	var results []checkResult

	hypePath := ""
	if err := findConfig(nil, nil); err == nil && configPath != "" {
		hypePath = configPath
	} else if path, err := config.LocateHYPEConfig(); err == nil {
		hypePath = path
	}

	if hypePath == "" {
		return []checkResult{{catInstallation, "HypeShell Configuration", statusError, "Not found", "shell.qml not found in any config path", doctorDocsURL + "#hype-configuration"}}
	}

	results = append(results, checkResult{catInstallation, "HypeShell Configuration", statusOK, "Found", hypePath, doctorDocsURL + "#hype-configuration"})

	shellQml := filepath.Join(hypePath, "shell.qml")
	if _, err := os.Stat(shellQml); err != nil {
		results = append(results, checkResult{catInstallation, "shell.qml", statusError, "Missing", shellQml, doctorDocsURL + "#hype-configuration"})
	} else {
		results = append(results, checkResult{catInstallation, "shell.qml", statusOK, "Present", shellQml, doctorDocsURL + "#hype-configuration"})
	}

	if doctorVerbose {
		installType := "Unknown"
		switch {
		case strings.Contains(hypePath, "/nix/store"):
			installType = "Nix store"
		case strings.Contains(hypePath, ".local/share") || strings.Contains(hypePath, "/usr/share"):
			installType = "System package"
		case strings.Contains(hypePath, ".config"):
			installType = "User config"
		}
		results = append(results, checkResult{catInstallation, "Install Type", statusInfo, installType, hypePath, doctorDocsURL + "#hype-configuration"})
	}
...
func checkConfigurationFiles() []checkResult {
	...
	configFiles := []struct{ name, path string }{
		{"settings.json", filepath.Join(configDir, hypeDir, "settings.json")},
		{"clsettings.json", filepath.Join(configDir, hypeDir, "clsettings.json")},
		{"plugin_settings.json", filepath.Join(configDir, hypeDir, "plugin_settings.json")},
		{"session.json", filepath.Join(utils.XDGStateHome(), hypeDir, "session.json")},
		{"hype-colors.json", filepath.Join(cacheDir, hypeDir, "hype-colors.json")},
	}
...
func checkSystemdServices() []checkResult {
	...
	legacyState := getServiceState("hype", true)
	if legacyState.exists {
		...
		results = append(results, checkResult{catServices, "hype.service", statusWarn, message, "Legacy upstream service still exists; HypeShell uses hype.service", doctorDocsURL + "#services"})
	}
